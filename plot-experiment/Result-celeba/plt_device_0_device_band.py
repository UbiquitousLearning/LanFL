...
def plot_accuracy_vs_server_time_leaf_lan_aware(weighted=False, plot_stds=False, figsize=(10, 8), title_fontsize=16, **kwargs):
    ...
    #          label='WAN-FL,BW=2',
    #          linestyle='-', color='red', linewidth=3)
    plt.plot([time_second/3600 for time_second in lan_device_2_acc_time[SERVER_TIME_KEY]], lan_device_2_acc_time[ACCURACY_KEY],
             label='LanFL,E=2,RL=10,$NC_s$=2',
             linestyle='--', color='orange', linewidth=3)
    plt.plot([time_second/3600 for time_second in lan_device_5_acc_time[SERVER_TIME_KEY]], lan_device_5_acc_time[ACCURACY_KEY],
             label='LanFL,E=2,RL=10,$NC_s$=5',
             linestyle='-.', color='green', linewidth=3)
    plt.plot([time_second/3600 for time_second in lan_device_10_acc_time[SERVER_TIME_KEY]], lan_device_10_acc_time[ACCURACY_KEY],
             label='LanFL,E=2,RL=10,$NC_s$=10',
             linestyle='-', color='red', linewidth=3)
    plt.plot([time_second/3600 for time_second in lan_device_20_bl_2_acc_time[SERVER_TIME_KEY]], lan_device_20_bl_2_acc_time[ACCURACY_KEY],
             label='LanFL,E=2,RL=10,$NC_s$=20',
             linestyle=':', color='purple', linewidth=3)
    # plt.plot([time_second/3600 for time_second in lan_device_20_bl_5_acc_time[SERVER_TIME_KEY]], lan_device_20_bl_5_acc_time[ACCURACY_KEY],
    #          label='LanFL,BL=5,NC=20',
    #          linestyle='-.', color='red', linewidth=3)
    ...
    _set_plot_properties(kwargs)
    plt.savefig('pdf/eval-time-device-band-celeba.pdf', bbox_inches='tight')
    plt.show()
...
if __name__ == '__main__':
    # data dir
    # study lan_bandwidth vs. device_num
    lan_device_2_dir = 'tune_NC/clients_5×2/ps/metrics/metrics_stat.csv'
    lan_device_5_dir = 'tune_NC/clients_5×5/ps/metrics/metrics_stat.csv'
    lan_device_10_dir = 'tune_NC/clients_5×10/ps/metrics/metrics_stat.csv'
    lan_device_20_bl_2_dir = 'tune_NC/clients_5×20/ps/metrics-1/metrics_stat.csv'
    lan_device_20_bl_5_dir = 'tune_NC/clients_5×20/ps/metrics-2/metrics_stat.csv'

    # get accuracy and std
    lan_device_2_acc_round = get_acc(load_data(lan_device_2_dir), NUM_ROUND_KEY, weighted=True)
    lan_device_2_acc_time = get_acc(load_data(lan_device_2_dir), SERVER_TIME_KEY, weighted=True)
    lan_device_5_acc_round = get_acc(load_data(lan_device_5_dir), NUM_ROUND_KEY, weighted=True)
    lan_device_5_acc_time = get_acc(load_data(lan_device_5_dir), SERVER_TIME_KEY, weighted=True)
    lan_device_10_acc_round = get_acc(load_data(lan_device_10_dir), NUM_ROUND_KEY, weighted=True)
    lan_device_10_acc_time = get_acc(load_data(lan_device_10_dir), SERVER_TIME_KEY, weighted=True)
    lan_device_20_bl_2_acc_round = get_acc(load_data(lan_device_20_bl_2_dir), NUM_ROUND_KEY, weighted=True)
    lan_device_20_bl_2_acc_time = get_acc(load_data(lan_device_20_bl_2_dir), SERVER_TIME_KEY, weighted=True)
    lan_device_20_bl_5_acc_round = get_acc(load_data(lan_device_20_bl_5_dir), NUM_ROUND_KEY, weighted=True)
    lan_device_20_bl_5_acc_time = get_acc(load_data(lan_device_20_bl_5_dir), SERVER_TIME_KEY, weighted=True)
